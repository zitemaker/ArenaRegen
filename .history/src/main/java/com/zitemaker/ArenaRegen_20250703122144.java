...
public class ArenaRegen extends JavaPlugin{
    ...
    public void saveRegionsAsync() {
        ...
        for (String regionName : regionsToProcess) {
            ...
            if (region == null || isRegionEmpty(region)) {
                // Silently skip non-existent or empty regions
                continue;
            }
    ...
    // Helper to check if a region is empty (no blocks, entities, banners, signs, or modified blocks)
    private boolean isRegionEmpty(RegionData region) {
        if (region == null) return true;
        // Use direct access to internal data structures for emptiness check
