...
public class ArenaRegen extends JavaPlugin{
    ...
    public void regenerateArenaSchedule(String arenaName) {
        ...
        // Ensure block data is loaded before proceeding
        regionData.ensureBlockDataLoaded().thenRun(() -> {
            ...
            Bukkit.getScheduler().runTaskTimer(this, task -> {
                ...
                if (currentSection >= sectionNames.size()) {
                    if (trackEntities) {
                        regionData.getEntityDataMap().thenAccept(entityDataMap -> {
            ...
            logger.info(ChatColor.RED + "Failed to load region data for '" + arenaName + "': " + e.getMessage());
            return null;
        });
    }
        ...
        regionData.ensureBlockDataLoaded().thenRun(() -> {
            ...
            Bukkit.getScheduler().runTaskTimer(this, task -> {
                ...
                if (currentSection >= sectionNames.size()) {
                    ...
                    regionData.getBannerStates().thenAccept(bannerStates -> {
                    ...
                    regionData.getSignStates().thenAccept(signStates -> {
            ...
            sender.sendMessage(prefix + ChatColor.RED + "Failed to load region data for '" + arenaName + "': " + e.getMessage());
            return null;
        });
    }
