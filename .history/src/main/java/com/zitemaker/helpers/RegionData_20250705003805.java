...
public class RegionData {
    ...
    public synchronized CompletableFuture<Void> ensureBlockDataLoaded() {
        if (isBlockDataLoaded) {
            LOGGER.info("[ArenaRegen] Block data already loaded for region in " + (datcFile != null ? datcFile.getName() : "unknown file"));
            return CompletableFuture.completedFuture(null);
        }
        if (blockDataLoadFuture != null && !blockDataLoadFuture.isDone()) {
            LOGGER.info("[ArenaRegen] Block data loading already in progress for region in " + (datcFile != null ? datcFile.getName() : "unknown file"));
            return blockDataLoadFuture;
        }
        
        LOGGER.info("[ArenaRegen] Starting block data loading for region in " + (datcFile != null ? datcFile.getName() : "unknown file"));
        
        blockDataLoadFuture = CompletableFuture.runAsync(() -> {
            if (isLoading) {
                LOGGER.severe("[ArenaRegen] Recursive loading detected for region in " + (datcFile != null ? datcFile.getName() : "unknown file"));
            ...
            if (!isBlockDataLoaded && datcFile != null) {
                LOGGER.info("[ArenaRegen] Checking if file exists: " + datcFile.getAbsolutePath());
                if (!datcFile.exists()) {
                    LOGGER.severe("[ArenaRegen] Datc file does not exist: " + datcFile.getAbsolutePath());
                    throw new RuntimeException("Datc file does not exist: " + datcFile.getAbsolutePath());
                }
                
                if (!datcFile.canRead()) {
                    LOGGER.severe("[ArenaRegen] Cannot read datc file: " + datcFile.getAbsolutePath());
                    throw new RuntimeException("Cannot read datc file: " + datcFile.getAbsolutePath());
                }
                
                LOGGER.info("[ArenaRegen] File size: " + datcFile.length() + " bytes");
                
                if (loadFailed) {
                    LOGGER.severe("[ArenaRegen] Block data loading previously failed for region in " + datcFile.getName());
                    throw new RuntimeException("Block data loading previously failed for region in " + datcFile.getName());
                }

                LOGGER.info("[ArenaRegen] Looking for world: '" + worldName + "'");
                World world = Bukkit.getWorld(worldName);
                if (world == null) {
                    LOGGER.severe("[ArenaRegen] World '" + worldName + "' not found for region in " + datcFile.getName());
                    LOGGER.info("[ArenaRegen] Available worlds: " + Bukkit.getWorlds().stream().map(World::getName).collect(java.util.stream.Collectors.joining(", ")));
                    loadFailed = true;
                    throw new RuntimeException("World '" + worldName + "' not found for region in " + datcFile.getName());
                }
                
                LOGGER.info("[ArenaRegen] World found: " + world.getName() + " (Environment: " + world.getEnvironment() + ")");
            } else {
                LOGGER.info("[ArenaRegen] Block data already loaded or no datc file for region");
            }
        }).thenCompose(v -> {
            // Now handle the actual loading asynchronously
            if (!isBlockDataLoaded && datcFile != null) {
                try {
                    isLoading = true;
                    LOGGER.info("[ArenaRegen] Starting loadFromDatc for " + datcFile.getName());
                    return loadFromDatc(datcFile).whenComplete((result, ex) -> {
                        ...
                        if (ex != null) {
                            ...
                            LOGGER.info("[ArenaRegen] Successfully loaded region data from " + datcFile.getName());
                        }
                    });
                } catch (Exception e) {
                    isLoading = false;
            ...
            if (ex != null) {
                LOGGER.severe("[ArenaRegen] Block data loading failed for region in " + (datcFile != null ? datcFile.getName() : "unknown file") + ": " + ex.getMessage());
                blockDataLoadFuture = null;
            } else {
                LOGGER.info("[ArenaRegen] Block data loading completed successfully for region in " + (datcFile != null ? datcFile.getName() : "unknown file"));
            }
        });
        return blockDataLoadFuture;
    }
