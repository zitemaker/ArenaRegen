...
public class RegionData {
    ...
    // --- Streaming Chunked File Write Support ---
    // For large regions, we stream each chunk directly to the .datc file to avoid high memory usage.
    // The file format remains compatible, but section count is written as a placeholder and fixed up at the end.

    /**
     * If true, all region saves will use streaming mode, regardless of size. Set via config or for testing.
     */
    public static boolean forceStreaming = false;
    ...
    /**
     * Begin streaming write to a .datc file. Writes the header and reserves space for section count.
     */
    public void startStreamingDatc(File datcFile) throws IOException {
        ...
        // Write header (same as in saveToDatc)
        ...
        // Write placeholder for section count (loader will count sections as it loads)
        streamingDos.writeInt(0);
        streamingSectionCount = 0;
    }

    /**
     * Write a chunk section to the streaming .datc file.
     * @param sectionName Name of the section (e.g. "chunk_0")
     * @param blocks Map of Location to BlockData for this chunk
     * @param datcFile The file being written (must match streamingDatcFile)
     * @param append Ignored (for API compatibility)
     */
    public void writeChunkToDatc(String sectionName, Map<Location, BlockData> blocks, File datcFile, boolean append) throws IOException {
    ...
    /**
     * Finalize the streaming .datc file: write entities, banners, signs, modified blocks, and fix up section count.
     */
    public void finalizeStreamingDatc(File datcFile, Map<Location, Map<String, Object>> entities, Map<Location, Map<String, Object>> banners, Map<Location, Map<String, Object>> signs, Map<Location, BlockData> modifiedBlocks) throws IOException {
        if (streamingDos == null || !datcFile.equals(streamingDatcFile)) throw new IOException("Streaming not started or file mismatch");
        // Write entities, banners, signs, modified blocks (same as saveToDatc)
        ...
        streamingDos.flush();
        streamingDos.close();
        // Now fix up the section count at the reserved position
        // (This is tricky with GZIP, so for now, we recommend reading the section count from the file on load, as the loader already does)
        streamingDos = null;
