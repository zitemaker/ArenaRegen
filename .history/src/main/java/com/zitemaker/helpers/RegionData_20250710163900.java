...
public class RegionData {
    ...
    public void addSection(String sectionName, Map<Location, BlockData> blocks) {
        ...
        // Use chunked processing for huge sections to prevent memory issues
        final int CHUNK_SIZE = 10000; // Process 10k blocks at a time
        ...
        for (int i = 0; i < blockEntries.size(); i += CHUNK_SIZE) {
            ...
            for (Map.Entry<Location, BlockData> entry : chunk) {
                ...
                // Only check for banners/signs if world is available and we're not processing a huge section
                if (world != null && blocks.size() < 1000000) {
                    BlockState state = world.getBlockAt(location).getState();
                    if (state instanceof Banner) {
                        Banner banner = (Banner) state;
                        Map<String, Object> bannerData = new HashMap<>();
            ...
            // Give GC a chance to clean up for huge sections
    ...
    public CompletableFuture<Void> saveToDatc(File datcFile) {
        ...
        // Use chunked copying to avoid memory issues with huge regions
    ...
    /**
     * Creates a chunked copy of the sectioned block data to avoid memory issues with huge regions
     */
    private Map<String, Map<Location, BlockData>> createChunkedCopy(Map<String, Map<Location, BlockData>> source) {
        ...
        final int CHUNK_SIZE = plugin.getConfig().getInt("general.huge-region-settings.chunk-size", 10000);
        ...
        for (Map.Entry<String, Map<Location, BlockData>> sectionEntry : source.entrySet()) {
            ...
            // Copy blocks in chunks to avoid memory issues
    ...
    private void writeSections(DataOutputStream dos, Map<String, Map<Location, BlockData>> sectionedBlockDataCopy) throws IOException {
        ...
        for (Map.Entry<String, Map<Location, BlockData>> entry : sectionedBlockDataCopy.entrySet()) {
            ...
            // Use smaller batch size for huge regions to prevent memory issues
            int batchSize = blocks.size() > 1000000 ? 500 : 1000; // Smaller batches for massive regions
            int count = 0;
            
            // Convert to list for chunked processing to avoid iterator memory issues
            List<Map.Entry<Location, BlockData>> blockEntries = new ArrayList<>(blocks.entrySet());
            final int CHUNK_SIZE = 5000; // Process 5k blocks at a time
            ...
            for (int i = 0; i < blockEntries.size(); i += CHUNK_SIZE) {
                ...
                // Give GC a chance to clean up for huge regions
                if (blocks.size() > 100000 && i % (CHUNK_SIZE * 5) == 0) {
                    System.gc();
                }
            }
