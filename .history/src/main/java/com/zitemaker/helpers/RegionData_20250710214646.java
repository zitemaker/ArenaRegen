...
public class RegionData {
    ...
    private Map<String, Map<Location, BlockData>> streamingSections = new ConcurrentHashMap<>();
    public static boolean forceStreaming;



    public RegionData(ArenaRegen plugin) {
        this.plugin = plugin;
        this.forceStreaming = plugin.getConfig().getBoolean("general.force-streaming", false);
    }
    ...
    public CompletableFuture<Void> startStreamingDatc(File datcFile) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                File parentDir = datcFile.getParentFile();
                ...
                // Don't write section count yet - we'll write it during finalization
    ...
    public CompletableFuture<Void> writeChunkToDatc(String sectionName, Map<Location, BlockData> blocks, File datcFile, boolean append) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                // Store the section in memory instead of writing directly
                streamingSections.put(sectionName, new ConcurrentHashMap<>(blocks));
    ...
    public CompletableFuture<Void> finalizeStreamingDatc(File datcFile, Map<Location, Map<String, Object>> entities, Map<Location, Map<String, Object>> banners, Map<Location, Map<String, Object>> signs, Map<Location, BlockData> modifiedBlocks) {
        return CompletableFuture.runAsync(() -> {
            try {
                ...
                // First, we need to rewrite the file with proper section count
                // Since we can't easily modify the beginning of a GZIP stream, we'll need a different approach
                // For now, let's write the section count at the end and modify the loader to handle this
                
                // Write a marker to indicate this is a streaming format
                streamingDos.writeInt(-1); // Special marker for streaming format
                streamingDos.writeInt(streamingSectionCount); // Write actual section count
