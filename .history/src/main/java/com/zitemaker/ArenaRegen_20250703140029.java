...
public class ArenaRegen extends JavaPlugin{
    ...
    public void regenerateArena(String arenaName, CommandSender sender) {
        ...
        // Check if block data is loaded (add isBlockDataLoaded() to RegionData if needed)
        if (!regionData.isBlockDataLoaded()) {
            ...
            regionData.ensureBlockDataLoaded().thenRun(() -> {
                Bukkit.getScheduler().runTask(this, () -> {
                    ...
                    // Retry regeneration (will re-add to regeneratingArenas)
                ...
                if (sender != null) {
                    sender.sendMessage(prefix + ChatColor.RED + " Failed to load region data: " + e.getMessage());
                }
                return null;
            });
            return;
