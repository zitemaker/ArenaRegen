...
public class ArenaRegen extends JavaPlugin{
    ...
    private void regenerateArena(String arenaName, CommandSender sender, int retryCount) {
        ...
        if (!regionData.isBlockDataLoaded()) {
            ...
            // Check if the arena file actually exists
            File expectedFile = new File(getDataFolder(), "arenas/" + arenaName + ".datc");
            logger.info("[ArenaRegen] Expected file path: " + expectedFile.getAbsolutePath());
            logger.info("[ArenaRegen] Expected file exists: " + expectedFile.exists());
            
            // If datcFile is null but the file exists, set it
            if (regionData.getDatcFile() == null && expectedFile.exists()) {
                logger.info("[ArenaRegen] Setting datcFile for arena '" + arenaName + "' to: " + expectedFile.getAbsolutePath());
                regionData.setDatcFile(expectedFile);
            }
