...
public class ArenaRegen extends JavaPlugin{
    ...
    public int arenaCreationTimeout;
    ...
    public void loadConfigValues() {
        ...
        this.arenaCreationTimeout = getConfig().getInt("general.arena-creation-timeout-seconds", 300);
        this.forceStreaming = getConfig().getBoolean("general.force-streaming", false);

        if (!getConfig().contains("general.large-region-settings.chunk-size")) {
            getConfig().set("general.large-region-settings.chunk-size", 10000);
        }
        if (!getConfig().contains("general.large-region-settings.write-batch-size")) {
            getConfig().set("general.large-region-settings.write-batch-size", 5000);
        }
        if (!getConfig().contains("general.large-region-settings.gc-frequency")) {
            getConfig().set("general.large-region-settings.gc-frequency", 100000);
        }
        if (!getConfig().contains("general.large-region-settings.progress-update-frequency")) {
            getConfig().set("general.large-region-settings.progress-update-frequency", 50000);
        }
        if (!getConfig().contains("general.large-region-settings.progress-update-interval-seconds")) {
            getConfig().set("general.large-region-settings.progress-update-interval-seconds", 5);
        }
        if (!getConfig().contains("general.arena-creation-timeout-seconds")) {
            getConfig().set("general.arena-creation-timeout-seconds", 300);
        }
        if (!getConfig().contains("general.force-streaming")) {
            getConfig().set("general.force-streaming", false);
        }

        try {
            getConfig().save(new File(getDataFolder(), "config.yml"));
        } catch (Exception e) {
            getLogger().warning("Could not save config.yml with new options: " + e.getMessage());
        }
    }
